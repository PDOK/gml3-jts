...
public final class ArcUtils {
  ...
  private static Coordinate[] orderCoordinates(Coordinate[] coordinates, GeometryFactory factory) {
    ...
    if (!counterClockWise) {
      /*
       * reverse the coordinates, otherwise a circle is created (the method that densifies the arc
       * will continue over the whole arc of the circle)
       */
  ...
  public static CoordinateSequence densify(Coordinate[] coordinates, GeometryFactory factory) {
    ...
    Coordinate[] result = SmallCircle.linearizeArc(orderedCoordinates[0].x, orderedCoordinates[0].y,
        orderedCoordinates[1].x, orderedCoordinates[1].y, orderedCoordinates[2].x,
        orderedCoordinates[2].y, tolerance);
