...
public class ArcLineString extends LineString {
  ...
  public boolean equalsExact(Geometry other, double tolerance) {
    ...
    for (int i = 0; i < points.size(); i++) {
      if (!equal(points.getCoordinate(i), otherArc.points.getCoordinate(i), tolerance)) {
        return false;
      }
    }
  ...
  public void normalize() {
    for (int i = 0; i < getDensifiedPoints().size() / 2; i++) {
      ...
      if (!getDensifiedPoints().getCoordinate(i).equals(getDensifiedPoints().getCoordinate(j))) {
        if (getDensifiedPoints().getCoordinate(i)
            .compareTo(getDensifiedPoints().getCoordinate(j)) > 0) {
