...
/**
 * <p>
 * GMLMultiVersionParserImpl class.
 * </p>
 *
 * @author Niek Hoogma
 *
 *         Parser which tries parsers for various GML versions. It tries the last success full
 *         parser first in order to speed up batch processing.
 *
...
public class GMLMultiVersionParserImpl implements GMLParser {
  ...
  /**
   * <p>
   * Constructor for GMLMultiVersionParserImpl.
   * </p>
   */
  ...
  public GMLMultiVersionParserImpl(double maximumArcApproximationError, final int srid) {
    ...
    // GML321GeotoolsParserImpl have a different arcApproximationError and is therefore not used
    // here.
  ...
  public Geometry toJTSGeometry(Reader reader) throws GML3ParseException {
    ...
    try {
      ...
      LOGGER.info("Not parseable using {}. Trying other GML parser versions. {} : {}", parserToTry,
          parserToTry, ex.getMessage());
      ...
      while (it.hasNext()) {
        ...
        if (!parser.equals(parserToTry)) {
          ...
          try {
            ...
            LOGGER.info(
                "Succesfully parsed using {}. Next geometry will also first be parsed with this parser",
                parser);
  ...
  public Geometry toJTSGeometry(String gml) throws GML3ParseException {
    ...
    try {
      ...
      LOGGER.info(
          "Not parseable using last-used parser {}. Trying other GML parser versions. {} : {}",
          parserToTry, parserToTry, ex.getMessage());
