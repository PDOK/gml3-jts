...
/**
 * <p>
 * GML321ToSurfaceConvertor class.
 * </p>
 *
 * @author GinkeM
 * @version $Id: $Id
...
public class GML321ToSurfaceConvertor {
  ...
  /**
   * <p>
   * Constructor for GML321ToSurfaceConvertor.
   * </p>
   *
   * @param geometryFactory a {@link org.locationtech.jts.geom.GeometryFactory} object.
   * @param gmlToLineConvertor a {@link nl.pdok.gml3.impl.gml3_2_1.converters.GML321ToLineConvertor}
   *        object.
   */
  public GML321ToSurfaceConvertor(GeometryFactory geometryFactory,
      GML321ToLineConvertor gmlToLineConvertor) {
  ...
  public Geometry convertMultiSurface(MultiSurfaceType surfaces) throws GeometryException {
    ...
    if (array != null) {
      List<JAXBElement<? extends AbstractSurfaceType>> arraySurfaceMembers =
          array.getAbstractSurface();
  ...
  public Geometry convertSurface(AbstractSurfaceType abstractSurface) throws GeometryException {
    if (abstractSurface instanceof SurfaceType) {
      ...
      for (int i = 0; i < patches.getAbstractSurfacePatch().size(); i++) {
        AbstractSurfacePatchType abstractPatch =
            patches.getAbstractSurfacePatch().get(i).getValue();
      ...
      for (int i = 0; i < polygonType.getInterior().size(); i++) {
        innerRings[i] = gmlToLineConvertor.translateAbstractRing(polygonType.getInterior().get(i));
      }
  ...
  public Polygon convertPolygonPatch(PolygonPatchType polygonPatch) throws GeometryException {
    ...
    for (int i = 0; i < polygonPatch.getInterior().size(); i++) {
      innerRings[i] = gmlToLineConvertor.translateAbstractRing(polygonPatch.getInterior().get(i));
   ...
   * <p>
   * This is different then lr.reverses() in that a copy is produced using a new coordinate
   * sequence.
   * </p>
   *
