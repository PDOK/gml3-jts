...
public class GML321ToPointConvertor {
  ...
  /**
   * <p>
   * Constructor for GML321ToPointConvertor.
   * </p>
   *
  ...
  public CoordinateArraySequence translateOrdinates(List<Double> ordinates, int dimension)
    ...
    if (ordinates.size() % dimension != 0) {
      throw new InvalidGeometryException(GeometryValidationErrorType.INVALID_COORDINATE,
          createCoordinateFromFirstTwoOrdinates(ordinates));
    }
  ...
  public CoordinateArraySequence translateOrdinates(DirectPositionListType posList)
      throws CoordinateMaxScaleExceededException, InvalidGeometryException {
  ...
  public Point convertPoint(PointType point) throws GeometryException {
    ...
    int dimension =
        pos.getSrsDimension() != null ? pos.getSrsDimension().intValue() : values.size();
    if (point.getPos() == null) {
      throw new DeprecatedGeometrySpecificationException("Geen post list voor ring gespecificeerd");
    }
    if (values.size() != dimension) {
      throw new InvalidGeometryException(
          GeometryValidationErrorType.POINT_INVALID_NUMBER_OF_ORDINATES, null);
    }
  ...
  public Geometry convertMultiPoint(MultiPointType multipointType) throws GeometryException {
    ...
    if (points.size() < 1) {
      throw new InvalidGeometryException(
          GeometryValidationErrorType.MULTI_POINT_DID_NOT_CONTAIN_MEMBERS, null);
